...
from xdl.xdl.constants import INERT_GAS_SYNONYMS
...
def remove_unused_modules(graph: Dict, graph_spec: Dict):
    ...
    """
    # Get the backbone, valve with pump
    ...
    for node, data in graph.nodes(data=True):
        ...
        if node_class in COMPONENT_CLASS_TYPE_DICT:
            ...
            if node_type not in used_modules:
                ...
                # CHeck the node's neighbors, 寻找节点的直连节点（上下游）
                for neighbor, neighbor_data in undirected_neighbors(
                    ...
                    if neighbor not in backbone:
                        ...
                        if (
                            neighbor_data["class"]
                            not in VACUUM_CLASSES + CHILLER_CLASSES
    ...
    # Remove any orphaned nodes no longer attached, 删除孤立节点
...
def remove_orphaned_nodes(graph: Dict):
    ...
    # Get orphaned nodes
    nodes_to_delete = [node for node in graph.nodes() if node not in used_nodes]
...
def apply_spec_to_template(
    ...
    """
    # 去除模板中未使用设备，从xdl中获取所需设备，对比模板中已有设备，去除未使用
...
def apply_buffer_flasks(
    ...
    for buffer_flask in buffer_flask_spec:
        ...
        for _ in range(n_required):
            ...
            # Add the flask
            add_buffer_flask(graph, buffer_flask_name, connected_valve, unused_ports[0])
...
def add_buffer_flask_edges(graph: dict, valve: str, flask: str, valve_port: str):
...
def apply_reagent_flasks_and_cartridges(
    ...
    # Get optimal arrangement of flasks，基于距离和体积，给定最优连接位置，考虑传输距离和时间
    ...
    for flask_name, position in optimal_arrangement.items():
        ...
        add_reagent_flask(
            graph, valve, port, flask_name, flask_reagent_dict[flask_name]
        )
    ...
    if inert_gas_flask:
        for valve in graph_valves(graph):
            if valve not in backbone:
                add_reagent_flask_edge(
                    graph, valve, flask_name, get_unused_valve_port(graph, valve)
                )
...
def add_reagent_flask_edge(graph: dict, valve: str, flask: str, valve_port: str):
...
def get_nearest_buffer_flask_valve(graph: dict, backbone_valve: str) -> str:
    ...
    while len([item for item in avoid if item in backbone_valves]) < len(
        backbone_valves
    ):
...
def get_buffer_flasks_on_valve(graph: dict, valve: str) -> Optional[List[str]]:
    ...
    for node in undirected_neighbors(graph, valve):
        ...
        if (
            graph.nodes[node]["class"] == "ChemputerFlask"
...
def apply_cartridge(graph, cartridge_spec, optimal_arrangement):
    ...
    for cartridge in cartridge_spec:
        ...
        if existing_cartridge_nodes:
            graph.nodes[existing_cartridge_nodes.pop()]["chemical"] = cartridge[
                "chemical"
            ]
            continue
        ...
        optimal_in = optimal_arrangement[cartridge_in_pseudo_flask_name(chemical)]
        optimal_out = optimal_arrangement[cartridge_out_pseudo_flask_name(chemical)]
