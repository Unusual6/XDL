...
from xdl_master.xdl.constants import JSON_PROP_TYPE
from xdl_master.xdl.errors import XDLSanityCheckError
...
if False:
    from xdl.steps import Step, XDLBase
...
class SanityCheck:
    ...
    def __init__(self, condition: bool, error_msg: str = "") -> None:
...
def steps_are_equal(step: Step, other_step: Step) -> bool:
    ...
    if "children" in step.properties and step.children:
        if "children" not in other_step.properties:
            return False
        if len(step.steps) != len(other_step.steps):
            return False
        for j, child in enumerate(step.steps):
            if not steps_are_equal(child, other_step.steps[j]):
                return False
    return True


def xdl_elements_are_equal(xdl_element: XDLBase, other_xdl_element: XDLBase) -> bool:
...
def reagent_volumes_table(reagent_volumes: dict[str, float]) -> str:
    ...
    reagent_volumes = [(reagent, volume) for reagent, volume in reagent_volumes.items()]
    ...
    # Create table
    return tabulate(table, headers=["Reagent", "Volume Used"], tablefmt="pretty")


def synthesis_id(procedure_attrs: dict) -> str | None:
    """Obtains and validates the 'id' attribute of the
    'Synthesis' section. If no id is found, the id attribute
    of the XDL object will be set to 'None'.

    Exceptions:
        Raises ValueError if an invalid value for the 'id'
         attribute has been specified.
    """
    # Regex the string for the synthsis id has to adhere to.
    rx = (
        r"^[^0-9.!#$%&'()*+,/;<=>?@^`{|}~\-\[\]\"\s]"
        r"[^!#$%&'()*+,/;<=>?@^`{|}~\[\]\"\s]*$"
    )
    synthesis_id = procedure_attrs.get("id", None)
    if synthesis_id is None:
        return None
    else:
        if re.search(rx, synthesis_id):
            return synthesis_id
        else:
            raise ValueError(
                """An invalid synthesis id has been """
                """provided. Tag names cannot contain """
...
def resolve_working_directory(xdl: str | dict, working_directory: str = None) -> str:
    """Set up working directory attribute. working_directory will be set
    according to the following algorithm:

        1. If working_directory is passed to the XDL constructor, it will be
            used as the working directory, otherwise:
        2. When the environment variable XDLPATH is defined, the
            working_directory will be set to XDLPATH, otherwise:
        3. When XDL is loaded from a XML/JSON syntax file, the
            working_directory will be set to the folder from which the file
            was loaded, otherwise:
        4. When XDL is loaded from a string or json dict, the
            working_directory will be set to the current working directory.

    Args:
        xdl (Union[str, Dict]): Path to .xdl, .xdlexe or .json XDL
            file, XML string, or JSON dict.
        working_directory (str, optional): working directory for loading XDL
            blueprints from other files. Defaults to None.
    """
    if working_directory:
        return working_directory

    elif "XDLPATH" in os.environ:
        return os.environ["XDLPATH"]
        ...
        if type(xdl) == str:
            if len(xdl) < 260:
                if os.path.exists(xdl):
                    return os.path.dirname(os.path.abspath(xdl))
            else:
                return os.getcwd()
