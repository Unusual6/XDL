...
from xdl_master.xdl.utils.sanitisation import (
    DEFAULT_PROP_LIMITS,
    convert_val_to_std_units,
    parse_bool,
)
...
class XDLBase:
    ...
    SPECIAL_PROPS = ["kwargs", "context"]

    def __init__(self, param_dict: Dict[str, Any], **kwargs) -> None:
        ...
        # `locals()`
        properties = {k: v for k, v in param_dict.items() if k in self.PROP_TYPES}

        if "context" in param_dict:
            properties["context"] = param_dict["context"]
            self.PROP_TYPES["context"] = None
            self.DEFAULT_PROPS["context"] = None
    ...
    def _load_properties(self, properties: Dict[str, Any]) -> None:
        ...
        for prop in self.PROP_TYPES:
            if prop in properties:
                self.properties[prop] = self._load_property(prop, properties[prop])
    ...
    def _clean_property(self, prop: str, value: Any) -> Any:
        ...
        if prop_type in [str, REAGENT_PROP_TYPE, VESSEL_PROP_TYPE]:
            ...
            if type(value) == str:
                return eval(value)  # noqa: DUO104 # nosec B307
    ...
    def _test_prop_limit(self, prop: str, value: Any) -> None:
        ...
        if not prop_limit.validate(value):
            # Value did not pass prop limit validation.
            raise XDLFailedPropLimitError(type(self).__name__, prop, value, prop_limit)
