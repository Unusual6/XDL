...
def conditional_human_readable(step, language_human_readable):
    ...
    # template str
    for fragment_identifier, condition_prop_dict in language_human_readable.items():

        # Ignore full template str
        if fragment_identifier != "full":

            # Match prop conditions
            for condition_prop, condition_val_dict in condition_prop_dict.items():

                # Get actual val
                condition_actual_val = step.properties[condition_prop]
                condition_actual_val_str = str(condition_actual_val).lower()

                sub_val = ""

                # Exact match
                if condition_actual_val_str in condition_val_dict:
                    sub_val = condition_val_dict[condition_actual_val_str]

                # Any match
                elif condition_actual_val is not None and "any" in condition_val_dict:
                ...
                if fragment_identifier not in step.properties:
                    formatted_properties[fragment_identifier] = sub_val
