...
class PropLimit:
    ...
    def __add__(self, otherPropLimit):
        return PropLimit(
            regex=f"({self.regex}|{otherPropLimit.regex})",
            hint=f"{self.hint} OR {otherPropLimit.hint}",
            default=f"{self.default} OR {otherPropLimit.hint}",
            enum=self.enum + otherPropLimit.enum,
        )
...
#: Pattern to match an Alphanumeric string without an underscore.
ALPHANUMERIC_PATTERN: str = r"^[A-Za-z0-9]*$"

#: Pattern to match scientific notation
POSITIVE_SCIENTIFIC_NOTATION_PATTERN: str = r"[+]?[\d]+\.?[\d]*[Ee](?:[-+]?[\d]+)"

#: Pattern combining scientific notation and positive float pattern
POSITIVE_SCIENTIFIC_FLOAT_PATTERN: str = (
    r"(" + POSITIVE_FLOAT_PATTERN + r"|" + POSITIVE_SCIENTIFIC_NOTATION_PATTERN + r")?"
)
...
ROTATION_SPEED_UNITS_PATTERN: str = r"(rpm|RPM)?"

#: Pattern to match all accepted length units, or empty string.
DISTANCE_UNITS_PATTERN: str = r"(nm|µm|mm|cm|m|km)?"

#: Pattern to match all accepted mol units, or empty string.
MOL_UNITS_PATTERN: str = r"(mmol|mol)?"

#: Pattern to match all accepted density units or empty string
DENSITY_UNITS_PATTERN: str = r"(g/cm3|g/ml|g/mL|mg/cm3|mg/ml|mg/mL|ug/cm3|ug/ml|ug/mL)?"

#: Pattern to match all accepted concentration units or an empty string
CONCENTRATION_UNITS_PATTERN: str = r"(mol/L|mol/l|mmol/l|mmol/L|M|mM|mmol/mL|nM)?"

#: Pattern to match all accepted units for molecular weights
MOLECULAR_WEIGHT_UNITS_PATTERN: str = r"(g/mol|mg/mol|ug/mol)?"

#: Pattern to match all accepted units for equivalence
EQUIVALENCE_UNIT_PATTERN: str = r"(equiv|equivs|equivalent|equivalents)?"

#: Pattern to match all accepted units for power
POWER_UNITS_PATTERN: str = r"(W|kW|MW)?"
...
VOLUME_PROP_LIMIT: PropLimit = PropLimit(
    regex=(
        r"^(all|("
        + POSITIVE_SCIENTIFIC_FLOAT_PATTERN
        + r"[ ]?"
        + VOLUME_UNITS_PATTERN
        + r")|("
        + POSITIVE_SCIENTIFIC_FLOAT_PATTERN
        + r"))$"
    ),
...
#: Prop limit for any props requiring a positive or negative float.
FLOAT_PROP_LIMIT: PropLimit = PropLimit(
    regex=FLOAT_PATTERN,
    hint='Expecting float value, e.g. "3", "-3.5"',
    default="0",
)
...
STIR_PURPOSE_PROP_LIMIT = PropLimit(enum=["dissolve"])
...
#: Prop limit for ``Reagent`` ``molecular_formula`` prop. Requires an alphanumeric string.
MOLECULAR_FORMULA_PROP_LIMIT: PropLimit = PropLimit(
    ALPHANUMERIC_PATTERN,
    hint="Expecting a molecular formula containing alphanumerics, e.g. C2H5OH",
    default="",
)

#: Prop Limit for ``Reagent`` ``molecular_weight`` prop. Requires a positive value
#: with mol units (g/mol)
MOLECULAR_WEIGHT_PROP_LIMIT: PropLimit = generate_quantity_units_pattern(
    POSITIVE_FLOAT_PATTERN,
    MOLECULAR_WEIGHT_UNITS_PATTERN,
    hint='Expecting positive weight value in mols e.g. "12 g/mol"',
    default="0 g/mol",
)

#: Prop Limit for ``Reagent`` ``density`` prop. Requires a positive value with density units.
DENSITY_PROP_LIMIT: PropLimit = generate_quantity_units_pattern(
    POSITIVE_FLOAT_PATTERN,
    DENSITY_UNITS_PATTERN,
    hint='Expecting postiive density value, e.g. "4.8 g/mL"',
    default="0 g/mL",
)

CONCENTRATION_PROP_LIMIT: PropLimit = generate_quantity_units_pattern(
    POSITIVE_FLOAT_PATTERN,
    CONCENTRATION_UNITS_PATTERN,
    hint='Expecting a positive concentration value, e.g. "0.5 mol/L"',
    default="0 mol/L",
)

AMOUNT_PROP_LIMIT: PropLimit = generate_quantity_units_pattern(
    POSITIVE_FLOAT_PATTERN,
    r"(" + MASS_UNITS_PATTERN + MOL_UNITS_PATTERN + EQUIVALENCE_UNIT_PATTERN + r")",
    hint="Expecting a positive amount value e.g. 1.2 equivs",
    default="1 equiv",
)
_fourteen_float: str = r"(14(?:[.][0]+)?)"
_ten_to_thirteen_float: str = r"([0-1][0-3](?:[.][0-9]+)?)"

PH_RANGE_PROP_LIMIT: PropLimit = PropLimit(
    (
        r"^("
        + _fourteen_float
        + "|"
        + _ten_to_thirteen_float
        + "|"
        + _zero_to_ten_float
        + ")$"
    ),
    hint="Expecting number between 0-14 representing a pH value, e.g. 12.46",
    default="7.0",
)

# Color prop limit for Irradiate step
COLOR_PROP_LIMIT = PropLimit(
    enum=[
        "red",
        "green",
        "blue",
...
#: Prop limit for power props.
POWER_PROP_LIMIT: PropLimit = generate_quantity_units_pattern(
    POSITIVE_FLOAT_PATTERN,
    POWER_UNITS_PATTERN,
    hint='Expecting power, e.g. "400 W".',
    default="200 W",
)

# Possible modes for distillation.
DISTILL_MODE_PROP_TYPE = PropLimit(enum=["standard", "kugelrohr"])

# Humidity sensor
HUMIDITY_PROP_LIMIT = PERCENT_RANGE_PROP_LIMIT

# Conductivity sensor
CONDUCTIVITY_PROP_LIMIT = POSITIVE_INT_PROP_LIMIT

# Prop limit for measurements. Add the corresponding prop limit whenever a new
# type of measurement is added.
MEASUREMENT_PROP_LIMIT = (
    TEMP_PROP_LIMIT  # DigiHotplate, ViscHotplate, BME280, RTD
    + PH_RANGE_PROP_LIMIT  # ViscHotplate
    # + CONDUCTIVITY_PROP_LIMIT  # conductivity sensor
    # + COLOR_PROP_LIMIT  # RGBSensor
    # + PRESSURE_PROP_LIMIT  # BME280
    # + HUMIDITY_PROP_LIMIT  # BME280
)
