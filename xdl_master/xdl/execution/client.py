...
class XDLExecutionClient:
    ...
    async def execute_step_device(self, graph, step_name, properties) -> None:
        ...
        """
        self._logger.info(f"Executing step: {step_name} {properties}\n")

        graph = self._platform.get_graph(json.loads(graph))
        ...
        step.execute_step(
            platform_controller,
            deps=None,
            locks=step.locks(platform_controller),
            tracer=[],
            step_indexes=[0],
        )
    ...
    def load_experiment(self, graph: str, xdlexe: str) -> None:
        ...
        """
        # Load graph
        self._graph = self._platform.get_graph(json.loads(graph))
    ...
    async def execute_step(self, step_uuid: str, resume: bool = False):
        ...
        if not self._platform_controller:
            self._logger.info("Can't execute. Platform controller not initialised.")
            return
    ...
    def _run_simulation(self) -> str:
        ...
        if not self._bound_platform_controller:
            simulation_platform_controller, error = self._get_platform_controller(
                self._graph, simulation=True
            )
        else:
        ...
        if not error:
            try:
                confirm_step_uuids = [
                    step["uuid"] for step in self._xdl_summary if step["confirm"]
                ]
                for step in self._xdl.steps:
                    if step.uuid not in confirm_step_uuids:
                        execute_step(simulation_platform_controller, step)
    ...
    def _get_log_file(self) -> str:
        ...
        return os.path.join(logs_folder, self.execution_key + ".txt")
    ...
    def _execute_base_step(self, base_step: Step) -> int:
        ...
        # Execute step
            ...
            try:
                execute_step(self._platform_controller, base_step)
...
def on_execute_step_device(data):
    client.execute_step_device(
        graph=data["graph"], step_name=data["step_name"], properties=data["properties"]
    )
...
def on_app_connect(data):
    ...
    ready to execute.
    """
    sio.emit("execlient-accepted-connection", {"execution_key": client.execution_key})
