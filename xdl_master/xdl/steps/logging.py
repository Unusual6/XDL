...
from xdl_master.xdl.steps.utils import pretty_props_table


def start_executing_step_msg(step, step_indexes: List[int] = None) -> str:
    ...
    human_readable = termcolor.colored(f"{step.human_readable()}.", attrs=["bold"])

    # Step name
    step_name = termcolor.colored(step.name, color="cyan", attrs=["bold"])

    # Step properties table
    prop_table = termcolor.colored(pretty_props_table(step.properties), color="cyan")

    # Combine all message parts and return
    return f"{first_line}\n{human_readable}\n{step_name}\n{prop_table}\n"


def finished_executing_step_msg(step, step_indexes: List[int] = None) -> str:
