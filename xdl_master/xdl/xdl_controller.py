    ...
    for step_name, step_class in step_library.items():
        ...
        if method_name not in attributes_dict:
            ...
            def step_method(self, step_class=step_class, **kwargs):
                block = [step_class(**kwargs, context=Context(platform=platform()))]
                self.executor.add_internal_properties(self.graph, steps=block)
                asyncio.run(block[0].execute(self.platform_controller))
...
def xdl_controller_init(
    self, platform: AbstractPlatform, platform_controller, graph_file
):
    """Initialise XDL controller."""
    self.platform = platform()
    self.graph = self.platform.get_graph(
        graph_file, device_modules=platform_controller.device_modules
    )
...
def XDLController(platform, platform_controller, graph_file):
    return make_xdl_controller(platform)(platform, platform_controller, graph_file)
